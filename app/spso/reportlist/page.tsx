...
const Page: React.FC = () => {
    ...
    const fetchReports = async () => {
        try {
            ...
            const sortedReports = response.data.data.sort((a: Report, b: Report) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    const downloadFile = async (reportId: string) => {
        try {
            const response = await axios.get(`http://localhost:8080/spso/periodic-report/${reportId}/download`, { responseType: 'blob' });
                ...
                {reports.map((report) => {
                    ...
                    return (
                        <li key={report.id} className="report-item">
                            <img src="https://img.icons8.com/color/48/000000/pdf.png" alt="PDF Icon" className="pdf-icon" />
                            <div className="report-info">
                                <h3 className="report-title">{formattedTitle}</h3>
                                <p>
                                    Tên file:{' '}
                                    <a className="report-title-file" onClick={() => downloadFile(report.id)}>
                                        {report.title}
                                    </a>
                                </p>
